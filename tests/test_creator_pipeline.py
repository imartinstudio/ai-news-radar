...
def make_story(index: int) -> dict:
    title = f"Codex coding agent release {index}"
    return {
        ...
        "source_count": 1,
        "score": 0.9,
        "latest_at": "2026-07-21T23:30:00Z",
        "primary_item": {
            "id": f"item-{index}",
            "title": title,
            "summary": "",
        ...
        "sources": [
            {
                ...
                "source_name": "Official AI Updates",
                "site_id": "official_ai",
                "published_at": "2026-07-21T23:30:00Z",
            }
        ],
    }
...
def test_candidate_pool_is_capped_at_30_and_edition_keeps_product_cap(tmp_path: Path):
    data = tmp_path / "data"
    data.mkdir()
    ...
    assert candidates["total_items"] == 30
    assert edition["total_items"] == 3
